...
3. Загружает информацию о команде (ID и имя) через `GET /api/v4/users/me/teams`
4. Если сессия валидна — переходит к списку каналов (состояние `AuthAuthenticated` содержит `User` и `teamName`)
5. Если нет — показывает экран авторизации
...
| `AuthAuthenticated` | Пользователь авторизован (содержит `User` и `teamName`) |
